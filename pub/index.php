...
Route::add('/upload', function() {
    ...
    header("Location: http://localhost/kacperhinz4gp/3.2/pub");
}, 'post');
...
Route::add('/admin/remove/([0-9]*)', function($id) {
    if(User::isAuth()) {
        Post::remove($id);
        header("Location: http://localhost/kacperhinz4gp/3.2/pub/admin");
    } else {
        http_response_code(403);
    }
});
...
Route::add('/like/([0-9]*)', function($post_id) {
    if(!User::isAuth()) {
        ...
        $like = new Vote($post_id, $user_id, true);
        $like->vote();
        header("Location: http://localhost/kacperhinz4gp/3.2/pub");
    }
});
...
Route::add('/dislike/([0-9]*)', function($post_id) {
    if(!User::isAuth()) {
        ...
        $user_id = $_SESSION['user']->getId();
        $like = new Vote($post_id, $user_id, false);
        $like->vote();
        header("Location: http://localhost/kacperhinz4gp/3.2/pub");
    }
});

Route::run('/kacperhinz4gp/3.2/pub');
